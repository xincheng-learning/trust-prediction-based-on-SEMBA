...
class TGNMemory(torch.nn.Module):
    ...
    def __init__(self, num_nodes: int, raw_msg_dim: int, memory_dim: int,
                 ...
                 aggregator_module: Callable, device='cpu'):
    ...
    def __reset_message_store__(self):
        i = self.memory.new_empty((0, ), dtype=torch.long, device=self.device)
    ...
    def __get_updated_memory__(self, n_id):
        ...
        msg = torch.cat([msg_s, msg_d], dim=0).data.clone()
        t = torch.cat([t_s, t_d], dim=0)
        aggr = self.aggr_module(msg, self.__assoc__[idx], t, n_id.size(0))

        # Get local copy of updated memory.
        # updated_memory = self.memory.data.clone()
        memory = self.gru(aggr, self.memory.data.clone()[n_id]) #self.memory.data.clone()[n_id])
...
class LastNeighborLoader(object):
    ...
    def insert(self, src, dst, times, weights):
